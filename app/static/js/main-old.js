    ...
    d3.json("static/maps/UI_output.json", function(d) {
        console.log("[in buyRent json d function ]" +d)
			  return {
                ...
                //console.log("in d3.values.buyrent.foreach function, appr_rate is: " + d.appr_rate)
                })
...
/*
logRangeMin = Math.log(minAppr)
logRangeMax = Math.log(maxAppr)
if (logRangeMin < 0) {
    logRangeMin = 0;
}
//rangeSlices = maxAppr/9;
//const colorZip = d3.scaleLog()
const colorZip = d3.scaleThreshold()
//const colorZip = d3.scaleLinear()
//.domain(d3.range(logRangeMin+1, logRangeMax))
//.domain(d3.range(Math.round(minAppr), Math.round(maxAppr)))
.domain(d3.range(parseInt(minAppr), parseInt(maxAppr)))
//.domain(d3.range(0,16))
//.domain([minAppr,maxAppr])
//.clamp(true)
.range(d3.schemeBlues[9]);


const cz2 = d3.scaleThreshold()
    .domain(d3.range(0,16))
    .range(d3.schemeBlues[9]);
 */
